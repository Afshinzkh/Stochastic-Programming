...
int **read_pgm(const char *filename, int imax, int jmax)
{
    ...
    if (imax != xsize || jmax != ysize)
    {
      fclose(input);
      ERROR("Dimensions between files don't match!");
    }
    ...
    pic = imatrix(0,xsize+1,0,ysize+1); /* fixed allocation of more elements than needed (I think) */
